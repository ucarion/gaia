...
use constants::{ELEVATION_DATA_OFFSET, ELEVATION_GRID_HEIGHT, ELEVATION_GRID_WIDTH};
use tile::TileKind;
use vertex::Vertex;
...
pub fn get_vertices() -> HashMap<TileKind, Vec<Vertex>> {
    ...
    result.insert(
        ...
        get_vertices_for_tile(include_bytes!(
            "../assets/generated/tiles/west_hemisphere/elevation.bin"
        )),
    );
    result.insert(
        TileKind::EastHemisphere,
        get_vertices_for_tile(include_bytes!(
            "../assets/generated/tiles/east_hemisphere/elevation.bin"
        )),
    );
    result
}

fn get_vertices_for_tile(elevation_data: &[u8]) -> Vec<Vertex> {
    ...
    while let Ok(elevation) = cursor.read_u16::<LittleEndian>() {
        ...
        result.push(get_vertex(x, y, actual_elevation));
...
fn get_vertex(x: u32, y: u32, elevation: i16) -> Vertex {
    let tex_coord = [
        ...
        y as f32 / ELEVATION_GRID_HEIGHT as f32,
    ];
